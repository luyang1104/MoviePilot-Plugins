...
class MediaSyncDel(_PluginBase):
    ...
    def sync_del_by_webhook(self, event: Event):
        ...
        # 媒体路径
        media_path = event_data.item_path or ""
        ...
        if not media_source or not media_id:
            logger.error(f"{media_name} 同步删除失败，未获取到有效媒体身份，请检查媒体库媒体是否刮削")
            return
    ...
    def sync_del_by_plugin(self, event: Event):
        ...
        # 注意：Scripter X/Emby webhook 传入的 item_isvirtual 是字符串（如 "False"），
        # 直接 if item_isvirtual 会把 "False" 判为真值，导致所有删除事件被静默跳过
        if str(item_isvirtual).strip().lower() in ("true", "1", "yes", "y", "on"):
            return
    ...
    def __sync_del(
        ...
        if not transfer_history:
            logger.warning(
                f"{media_type} {media_name} 未获取到可删除数据，请检查路径映射或媒体身份是否正确")
            return

        if delete_time:
            latest_his = max(transfer_history, key=lambda x: x.date)
            if delete_time < latest_his.date:
                logger.warning(f"忽略删除 {msg}, 整理时间: {latest_his.date}, 发生在删除事件: {delete_time} 之后")
                return

        logger.info(f"开始同步删除 {msg}, 获取到 {len(transfer_history)} 条转移记录")
        if not media_source or not media_id:
            for transferhis in transfer_history:
                media_source, media_id = resolve_media_identity(transferhis)
        ...
        for transferhis in transfer_history:
            ...
            if not title or title not in media_name:
                logger.warning(
        ...
        history.append({
            ...
            "image": poster_image,
            "del_time": now,
            "unique": f"{media_name}:{build_media_key(media_source, media_id) or 'unknown'}:{now}"
        })
            ...
            for parent_path in file_path.parents:
                ...
                if str(parent_path.parent) != str(file_path.root):
                    ...
                    if not SystemUtils.exits_files(parent_path, settings.RMT_MEDIAEXT):
                        ...
                        logger.warning(f"本地空目录 {parent_path} 已删除")

    def __get_transfer_his(self, media_type: str, media_name: str, media_path: str,
                           media_source: Optional[MediaSource], media_id: Optional[str],
                           season_num: Optional[str], episode_num: Optional[str]):
        """
        查询转移记录
        """
        # 季数
        ...
        if mtype == MediaType.MOVIE:
            ...
            if not media_source or not media_id:
                return msg, []
            transfer_history = self._transferhis.get_by(
                media_source=media_source,
                media_id=media_id,
                mtype=mtype.value,
                dest=media_path,
